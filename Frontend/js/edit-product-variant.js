...
// Verify admin access and load product on page load
...
async function loadProductData() {
    ...
    try {
        ...
        if (productData.colorVariants && productData.colorVariants.length > 0) {
            productData.colorVariants.forEach((variant, index) => {
...
function addColorVariant(existingVariant = null) {
    const container = document.getElementById('variantsContainer');
    const variantId = variantCount++;
    
    const variantHTML = `
        <div class="variant-item" id="variant-${variantId}" data-original-id="${existingVariant ? existingVariant._id : ''}">
            <div class="variant-header">
                <h3 class="variant-title">Color Variant ${variantId + 1}</h3>
                ${variantCount > 1 ? `<button type="button" class="remove-variant-btn" onclick="removeVariant(${variantId})">Remove</button>` : ''}
            </div>
            ...
            <div class="color-selector-group">
                ...
                <select class="color-selector" id="color-${variantId}" required>
                    ...
                    ${AVAILABLE_COLORS.map(color => `
                        <option value="${color.value}" data-hex="${color.hex}" ${existingVariant && existingVariant.color === color.value ? 'selected' : ''}>${color.name}</option>
                    `).join('')}
                </select>
            </div>
            ...
            <div class="variant-images-section">
                <div class="variant-image-box">
                    <div class="variant-image-preview" onclick="document.getElementById('mainImage-${variantId}').click()">
                        <img id="mainImagePreview-${variantId}" src="${existingVariant && existingVariant.images?.mainImage ? `${API_URL.replace('/api', '')}/${existingVariant.images.mainImage}` : '../User panel images/icons/upload-placeholder.png'}" alt="Main Image">
                    </div>
                    <div class="variant-image-label">Main Image *</div>
                    <input type="file" id="mainImage-${variantId}" accept="image/*" style="display: none;" 
                           onchange="previewImage(this, 'mainImagePreview-${variantId}')">
                </div>

                <div class="variant-image-box">
                    <div class="variant-image-preview" onclick="document.getElementById('additionalImage1-${variantId}').click()">
                        <img id="additionalImage1Preview-${variantId}" src="${existingVariant && existingVariant.images?.additionalImages?.[0] ? `${API_URL.replace('/api', '')}/${existingVariant.images.additionalImages[0]}` : '../User panel images/icons/upload-placeholder.png'}" alt="Additional 1">
                    </div>
                ...
                <div class="variant-image-box">
                    <div class="variant-image-preview" onclick="document.getElementById('additionalImage2-${variantId}').click()">
                        <img id="additionalImage2Preview-${variantId}" src="${existingVariant && existingVariant.images?.additionalImages?.[1] ? `${API_URL.replace('/api', '')}/${existingVariant.images.additionalImages[1]}` : '../User panel images/icons/upload-placeholder.png'}" alt="Additional 2">
                    </div>
                ...
                <div class="variant-image-box">
                    <div class="variant-image-preview" onclick="document.getElementById('additionalImage3-${variantId}').click()">
                        <img id="additionalImage3Preview-${variantId}" src="${existingVariant && existingVariant.images?.additionalImages?.[2] ? `${API_URL.replace('/api', '')}/${existingVariant.images.additionalImages[2]}` : '../User panel images/icons/upload-placeholder.png'}" alt="Additional 3">
                    </div>
            ...
            <div class="variant-sizes-grid">
                <div class="size-input-group">
                    <label>Size S</label>
                    <input type="number" id="sizeS-${variantId}" min="0" value="${existingVariant ? existingVariant.sizeQuantities?.S || 0 : 0}" required>
                </div>
                <div class="size-input-group">
                    <label>Size M</label>
                    <input type="number" id="sizeM-${variantId}" min="0" value="${existingVariant ? existingVariant.sizeQuantities?.M || 0 : 0}" required>
                </div>
                <div class="size-input-group">
                    <label>Size L</label>
                    <input type="number" id="sizeL-${variantId}" min="0" value="${existingVariant ? existingVariant.sizeQuantities?.L || 0 : 0}" required>
                </div>
                <div class="size-input-group">
                    <label>Size XL</label>
                    <input type="number" id="sizeXL-${variantId}" min="0" value="${existingVariant ? existingVariant.sizeQuantities?.XL || 0 : 0}" required>
                </div>
                <div class="size-input-group">
                    <label>Size 2XL</label>
                    <input type="number" id="size2XL-${variantId}" min="0" value="${existingVariant ? existingVariant.sizeQuantities?.XXL || 0 : 0}" required>
                </div>
            </div>
        </div>
    `;
...
document.getElementById('editProductForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        for (let i = 0; i < variantElements.length; i++) {
            ...
            // Only add new images if user uploaded them
        ...
        const response = await fetch(`${API_URL}/products/${productId}/with-variants`, {
            method: 'PUT',
            headers: {
                'Authorization': `Bearer ${token}`
            },
